...
export async function PATCH(
  ...
  try {
    ...
    if (!linkToUpdate) {
      return NextResponse.json({ message: "Link not found or permission denied." }, { status: 404 });
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        metaData.image = $('meta[property="og:image"]').attr("content") || $('meta[property="og:image:url"]').attr("content") || "";
      }
    } catch (scrapeError) {
      console.warn(`Failed to re-scrape ${originalUrl}:`, scrapeError);
    }
